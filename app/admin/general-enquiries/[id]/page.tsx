...
import { ArrowLeft, Mail, Phone, MessageSquare, CheckCircle, Trash } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function GeneralEnquiryDetailPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const response = await fetch(`/api/general-enquiries/${enquiryId}`)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push("/admin/general-enquiries")
    } catch (err) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64">
        ...
        <Button asChild variant="outline">
          <Link href="/admin/general-enquiries">Back to Enquiries</Link>
        </Button>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <Button asChild variant="outline" size="sm">
            <Link href="/admin/general-enquiries">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back to Enquiries
            </Link>
          </Button>
          <h1 className="text-3xl font-bold">General Enquiry Details</h1>
        </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="bg-muted p-4 rounded-lg">
                    <p className="font-medium">{getSubjectLabel(enquiry.subject)}</p>
                  </div>
                </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4">
                <Button asChild className="w-full" variant="outline">
                  <Link href={`mailto:${enquiry.email}?subject=RE: Your Enquiry at The Pearl Hotel`}>
                    Reply via Email
                  </Link>
                </Button>
              </div>
