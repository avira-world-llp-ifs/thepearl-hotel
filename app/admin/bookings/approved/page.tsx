...
export default async function ApprovedBookingsPage() {
  // Instead of using bookingService.getByStatus("approved")
  // Get all bookings and filter them in JavaScript
  const allBookings = await bookingService.getAll()
  const bookings = allBookings.filter(
    (booking) => booking.bookingStatus === "approved" || booking.status === "approved",
  )
  ...
  return (
    <div className="md:pt-0 pt-10">
      <h1 className="text-3xl font-bold mb-6">Approved Bookings</h1>

      <div className="bg-background rounded-lg border shadow-sm">
        {!enrichedBookings || enrichedBookings.length === 0 ? (
          <div className="p-6 text-center">
            <p className="text-muted-foreground mb-4">No approved bookings found.</p>
            <Button asChild>
              <Link href="/admin/bookings/new">Create New Booking</Link>
            </Button>
          </div>
        ) : (
